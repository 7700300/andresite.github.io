// ==UserScript==
// @name         forest_fant_cocna
// @namespace    http://tampermonkey.net/
// @version      1
// @description  try to take over the world!
// @author       You
// @match        https://newforest.apeha.ru/gates.html*
// @require      http://code.jquery.com/jquery-1.12.4.min.js
// @require      https://raw.githubusercontent.com/spriteololo/tess/master/tess.js
// @grant        GM_addStyle
// ==/UserScript==
...
var copperProb = {forward : 36, side : 24, radius : 20}
...
function startBtnClicked() {
    ...
    startLoop()
    startGraph()
    top.frames["d_act"].startChatChecker()
}

top.frames["d_act"].startChatChecker = function startChatChecker(){
    top.frames["d_act"].stopChatChecker()
    chatIntervalId = setInterval(function() {startChatCheck()}, 30000);
}

top.frames["d_act"].stopChatChecker = function stopChatChecker(){
    if(chatIntervalId != 0) {
...
top.frames["d_act"].showTravaGraph = function showTravaGraph(){
    canvTravaIntervalId = setInterval(function() {clearDots(); startTravCanv()}, 500);
}

function startTravCanv() {
    var travaArr = getAllItemsInRadius(13, "trava");

    byIdFr("d_act", "canvas").parentNode.style.overflow = "hidden";

    travaArr.forEach(id => createDot(id, "T"))
}

top.frames["d_act"].hideTravaGraph = function hideTravaGraph(){
    clearInterval(canvTravaIntervalId);
    clearDots()
}

function startChatCheck() {
    var myNick = top.frames["d_act"].global_data.my_group.sostav.leader.nick
    if(top.frames["d_chat"].document.getElementById("messages").innerHTML.includes(myNick)) {
        clearInterval(chatIntervalId)
        chatIntervalId = 0
        blink()
    }
...
function blink() {
    blinkInterval = setInterval(function() {
        setBackgroundColorEverywhere("rgb(255, 0, 0)")
        setTimeout(function(){setBackgroundColorEverywhere("#e3f2c6")}, 1000)
    }, 2000)
}

function setBackgroundColorEverywhere(colorStr) {
    top.frames["d_pers"].document.body.style.background = colorStr
    top.frames["d_act"].document.body.style.background = colorStr
    top.frames["d_chat"].document.body.style.background = colorStr
    top.frames["d_chatact"].document.body.style.background = colorStr
    top.frames["d_pers"].document.body.style.background = colorStr
    top.frames["d_ulist"].document.body.style.background = colorStr
    top.frames["d_menu"].document.body.style.background = colorStr
}
    ...
    if(timerCountDown == "-1") { // Timer is off
        ...
        if(overlayResponse == "Вы должны иметь в руках Топор лесоруба") {
            ...
            var promise = new Promise(function (resolve) {
                jQuery.get('https://newforest.apeha.ru/bag_type_17.chtml', function (response) {
                    var re = new RegExp('<img width="75" height="50" src="https:\/\/resources\.apeha\.ru\/upload\/1_962\.gif" title="Топор лесоруба">.+\\n.+\\n.+\\n<form method=post><input type=hidden name=actUser-Wear value=(\\d+)>', 'gm');
                    var match = re.exec(response);
                    if (match && match[1]) {
                        var itemId = +match[1];

                        setTimeout(function () {
                            jQuery.post('https://newforest.apeha.ru/bag_type_17.chtml', { 'actUser-Wear': itemId }).then(function () {
                                resolve();
                            });
                        }, 6000);
                    }
            ...
            if(overlayResponse == "Вы травмированы. Для работы необходимо вылечить травмы.") {
                ...
                var promise = new Promise(function (resolve) {
                    if(healId !== undefined) {
                        jQuery.post('https://newforest.apeha.ru/ability_type_common.chtml', { 'actUser-UseCast': healId }).then(function () {
                            resolve();
                        });
                    } else {
                    ...
                    case ("Nothing_forward"): {
                        ...
                        if (overlayResponse == "Ничего не найдено" || overlayResponse == "Not overlayed") {
                            ...
                            if(hitCount < numberOfSearches() &&
                                (isThere5Possible() || searchCopper && getIgnoredItemById(currentStoneId, getStoneTypeById(currentStoneId)).perc < 100 )) {
                                log.v("SEARCH")
        ...
        if(Number.isInteger(timerCountDown)) {
            ...
            (timerCountDown > 150 ? (timerCountDown - 150) : timerCountDown) * 1000 + getRandom(1200, 5000)) );
        }
    }
    log.v("FINISH #" + count)
    count++;
...
function getIgnoredItemById(_id, type) {
    var item = isInIgnoredItemById(_id) ? ignoredItems.find(item => item.id == _id) : {id: _id, perc: 0}
...
function createNavSelector(){
    if (top.frames["d_pers"].document.getElementsByTagName('body')[0]!=null) {
        ...
        var titlediv2 = createMyElement(pers_f, "div", "chatTitle", "", "", "", "<b>Чат:</b> ");
        var startChat = createNewButton(pers_f, "goAwayTrue", "width:100%!important;", "top.frames[\"d_act\"].startChatChecker()", "Чат", "width:100%;"); //TODO
        var stopChat = createNewButton(pers_f, "goAwayFalse", "width:100%!important;", "top.frames[\"d_act\"].stopChatChecker()", "Остановить чат", "width:100%;"); //TODO
        navkords.appendChild(titlediv2);
        navkords.appendChild(startChat);
        navkords.appendChild(stopChat);
        var titlediv3 = createMyElement(pers_f, "div", "Trava", "", "", "", "<b>Трава:</b> ");
        var showTrava = createNewButton(pers_f, "showTrava", "width:100%!important;", "top.frames[\"d_act\"].showTravaGraph()", "Показать траву", "width:100%;"); //TODO
        var hideTrava = createNewButton(pers_f, "hideTrava", "width:100%!important;", "top.frames[\"d_act\"].hideTravaGraph()", "Спрятать траву", "width:100%;"); //TODO
        navkords.appendChild(titlediv3);
        navkords.appendChild(showTrava);
        navkords.appendChild(hideTrava);
...
var awayInterval = 0;
var freeCounter = 0;
function awayChecker() {
    if(forest_frame.global_data.groups.filter(item => item != null && item != 0).length > 1) {
        var allOtherGroups = forest_frame.global_data.groups.filter(item => item != null && item != 0).filter(item => item.id != global_data.my_group.id)
        if(allOtherGroups[0] != null && Math.abs(allOtherGroups[0].posx - getMyPositionAndDirection().x) < 20 && Math.abs(allOtherGroups[0].posy - getMyPositionAndDirection().y) < 20) {
            freeCounter = 0
            log.e("run from " + allOtherGroups[0].sostav.leader.nick)
            if (global_data.wait_event == 3 || global_data.wait_event == 4){
                forest_frame.Client.send('actNewMaps-CancelEvent=3')
                stopScript()
            }
            if (global_data.wait_event == 2){
                forest_frame.Client.send('actNewMaps-CancelEvent=2')
                stopScript()
            }
            var myPos = getMyPositionAndDirection()
            var awayPos = {x: allOtherGroups[0].posx, y: allOtherGroups[0].posy}
            var localCoordinatesToGo = {}
            localCoordinatesToGo.x = (myPos.x - awayPos.x) < 13 && (myPos.x - awayPos.x) > -13 ? (myPos.x - awayPos.x) : ((myPos.x - awayPos.x) < 0 ? -12 : 12)
            localCoordinatesToGo.y = (myPos.y - awayPos.y) < 13 && (myPos.y - awayPos.y) > -13 ? (myPos.y - awayPos.y) : ((myPos.y - awayPos.y) < 0 ? -12 : 12)
            goToGlobalCoordinates(myPos.x + localCoordinatesToGo.x, myPos.y + localCoordinatesToGo.y)
            return;
        }
    }
...
top.frames["d_act"].goAwayTrue = function goAwayTrue(){
    if(awayInterval == 0) {
        log.i("awayChecker turned on")
        awayInterval = setInterval(awayChecker, 1000)
    }
}

top.frames["d_act"].goAwayFalse = function goAwayFalse(){
    clearInterval(awayInterval)
    log.i("awayChecker turned off")
    awayInterval = 0
}

var NavObjects = [
    {name :"Не выбрано", latname : "тщту", cordx : "", cordy : "", ofsetx: 0, ofsety: 0, obglocation : ""},
    {name :"Альпийский Одуванчик", latname : "Fort", cordx : 1568, cordy : 2314, ofsetx: 6, ofsety: 6, obglocation : "Альпийские луга"},
    {name :"Заначка Аутлавс", latname : "Fort", cordx : 2025, cordy : 3634, ofsetx: 6, ofsety: 6, obglocation : "Заводь среднеморская"},
...
function createDot(id, text) {
    ...
    var rrr = createMyElement(forest_f, "div", "dot-" + id, "", "display:block;position: absolute; z-index:2; width:35px; height:35px; top:0%;left:0%;margin-top:" + (dy * 35) + "px; margin-left:" + (dx * 35) + "px;background-color: #ffffff45;" + (currentStoneId == id ? "color: yellow;" : (current5.includes(id) ? "color: green;" : "color: red;")), "", text);
...
fetch("https://newforest.apeha.ru/interface/codeimage.fpl/" + data.ci).then(data1 => data1.blob()).then(res => {
